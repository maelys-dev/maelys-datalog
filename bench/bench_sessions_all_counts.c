/* Proof-only batch driver derived from bench_sessions.c. Timing driver is unchanged. */
...
int main(int argc, char **argv) {
    ...
    for (unsigned size = 0; size < sizeof(sizes) / sizeof(sizes[0]); ++size) {
        ...
        for (unsigned policy = 0; policy < 2; ++policy) {
            ...
            snprintf(count_label, sizeof(count_label), "%s/%s/%s/%s", policy_name, orders[order], value_name, sizes[size]);
#endif
...
#ifdef MAELYS_BENCH_COUNT
    if (selected != 200) { fprintf(stderr, "expected all 200 diagnostic cases\n"); return 2; }
#endif
    return 0;
}
