...
int main(int argc, char **argv) {
    ...
    if (layout_path) {
        FILE *layout = fopen(layout_path, "w"); require_true(layout != NULL, "layout output");
        write_layout(layout);
        fprintf(layout, "address.context,%" PRIuPTR "\naddress.ruleset,%" PRIuPTR "\naddress.edb,%" PRIuPTR "\n",
                (uintptr_t)&s_bench_ctx, (uintptr_t)&s_bench_ctx.ruleset, (uintptr_t)&s_bench_ctx.edb);
        require_true(fclose(layout) == 0, "close layout");
    }
