...
size=${DIAGNOSTIC_SIZE:-2048}
case "$size" in 1024|2048) ;; *) echo 'DIAGNOSTIC_SIZE must be 1024 or 2048' >&2; exit 2;; esac
...
for role in A B C; do
  ...
  for pad in 0 16 64 256; do
    ...
    "$binary" layout "$output/$role-$pad.layout.csv" "$size"
...
for role in A B C; do
  for pass in 1 2 3 4; do
    "$workspace/bin-$role/diagnostic-0" time "$output/$role-aa-$pass.csv" "$size"
  done
done
for pass in 1 2; do
  for pad in 0 16 64 256; do
    for role in A B C; do
      "$workspace/bin-$role/diagnostic-$pad" time "$output/$role-$pad-$pass.csv" "$size"
    done
  done
done
  ...
  for pad in 0 16 64 256; do
    for pass in 1 2; do
      ...
      valgrind --tool=callgrind --collect-atstart=no --dump-instr=yes --error-exitcode=3 \
        --callgrind-out-file="$prefix.out" "$workspace/bin-$role/diagnostic-$pad" count "$prefix.csv" "$size" \
        2> "$prefix.log"
