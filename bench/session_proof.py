        ...
        for role in ("A", "B"):
            ...
            for repeat in (1, 2):
                ...
                for path in paths:
                    ...
                    function_costs = exclusive_dump(text)
                    annotated_costs = annotated(annotation)
                    # Engine functions must also agree with the established
                    # annotate output; zero-call startup arcs are not engine work.
                    for name, values in function_costs.items():
                        if name.startswith(("maelys_", "solve_", "evaluate_", "aggregate_")) and values != annotated_costs.get(name, dict.fromkeys(EVENTS, 0)):
                            raise ValueError("raw/annotated engine function costs disagree")
