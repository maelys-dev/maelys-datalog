...
static int test_datalog_headers_owner_types_available(void) {
     ...
     * VERSION_STRING must be exactly MAJOR.MINOR.PATCH[-PRERELEASE]. */
    {
        char composed[64];
        if (MAELYS_DATALOG_VERSION_PRERELEASE[0] != '\0') {
            snprintf(composed, sizeof composed, "%d.%d.%d-%s",
                     MAELYS_DATALOG_VERSION_MAJOR, MAELYS_DATALOG_VERSION_MINOR,
                     MAELYS_DATALOG_VERSION_PATCH, MAELYS_DATALOG_VERSION_PRERELEASE);
        } else {
            snprintf(composed, sizeof composed, "%d.%d.%d",
                     MAELYS_DATALOG_VERSION_MAJOR, MAELYS_DATALOG_VERSION_MINOR,
                     MAELYS_DATALOG_VERSION_PATCH);
        }
        TEST_ASSERT_EQUAL_STRING(composed, MAELYS_DATALOG_VERSION_STRING);
    }
