"""Document access, atomic inputs and lifecycle of the public Python binding."""
...
import maelys_datalog as binding
...
# Reuse the same immutable declaration across tests: the native domain registry
# is process-wide and bounded, with no eviction.
...
class PythonBindingTest(unittest.TestCase):
    def test_document_access_expected_answers(self):
        answers, rows = evaluate(binding, "python_document_access")
        self.assertEqual(answers, EXPECTED)
        self.assertEqual(set(rows), {("alice", "roadmap.pdf"), ("bob", "roadmap.pdf")})

    def test_two_open_results_can_be_queried(self):
        with binding.Engine() as engine:
            domain = "python_document_access"
            engine.register_domain(domain, predicates(binding))
            ruleset = engine.load_inline_ruleset(domain, "documents.main", POLICY)
