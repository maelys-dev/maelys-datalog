...
static int test_maelys_py_bind_copies_common_declarations(void) {
    TEST_BEGIN();
    ...
    TEST_ASSERT_EQUAL((int)MAELYS_OK,
        maelys_py_find_domain("py_owned_declaration", &view, 1u,
                             &count, &found, &inspectable), "%d");
    TEST_ASSERT_TRUE(found && inspectable && count == 1u);
    if (!view.name) return 1;
    TEST_ASSERT_EQUAL_STRING("owned_predicate", view.name);
...
int main(int argc, char **argv) {
    ...
    const test_case_t cases[] = {
        {"maelys_py_bind/copies_common_declarations",
         TEST_MODE_NON_BLOCKING, test_maelys_py_bind_copies_common_declarations},
