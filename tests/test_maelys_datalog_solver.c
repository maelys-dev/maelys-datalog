...
static int init_solver_test_ruleset(maelys_datalog_internal_ruleset_t *r) {
    memset(r, 0, sizeof(*r));
    ...
    if (rc != MAELYS_OK) return rc;
    static const maelys_datalog_predicate_t test_defs[] = {
...
static int make_ruleset(maelys_datalog_internal_ruleset_t *r, const char *src) {
    int rc = init_solver_test_ruleset(r);
    if (rc != MAELYS_OK) return rc;
    return maelys_datalog_parse_ruleset(r, src, strlen(src));
}

static int parse_solver_test_ruleset_ex(const char *src, maelys_datalog_internal_diagnostic_t *diag) {
...
static maelys_datalog_internal_term_t sym_term(maelys_datalog_internal_ruleset_t *r, const char *s) {
...
static void add_parent(maelys_datalog_internal_ruleset_t *r, maelys_datalog_internal_edb_t *edb, const char *a, const char *b) {
    maelys_datalog_internal_term_t terms[2] = {sym_term(r, a), sym_term(r, b)};
    (void)maelys_datalog_edb_add_fact(edb, "parent", terms, 2);
}

static void add_symbol_unary(maelys_datalog_internal_ruleset_t *r,
                             maelys_datalog_internal_edb_t *edb,
                             const char *predicate,
                             const char *arg) {
    maelys_datalog_internal_term_t term = sym_term(r, arg);
    (void)maelys_datalog_edb_add_fact(edb, predicate, &term, 1);
}

static void add_int_unary(maelys_datalog_internal_edb_t *edb, const char *predicate, long long arg) {
    maelys_datalog_internal_term_t term = {.kind = MAELYS_DATALOG_TERM_INT};
    term.as.integer = arg;
    (void)maelys_datalog_edb_add_fact(edb, predicate, &term, 1);
}

static void add_int_symbol_binary(maelys_datalog_internal_ruleset_t *r,
                                  maelys_datalog_internal_edb_t *edb,
                                  const char *predicate,
                                  long long lhs,
                                  const char *rhs) {
...
static void add_symbol_symbol_binary(maelys_datalog_internal_ruleset_t *r,
                                     maelys_datalog_internal_edb_t *edb,
                                     const char *predicate,
                                     const char *lhs,
                                     const char *rhs) {
...
static void add_symbol_int_binary(maelys_datalog_internal_ruleset_t *r,
                                  maelys_datalog_internal_edb_t *edb,
                                  const char *predicate,
                                  const char *lhs,
                                  long long rhs) {
...
static void add_symbol_bool_binary(maelys_datalog_internal_ruleset_t *r,
                                   maelys_datalog_internal_edb_t *edb,
                                   const char *predicate,
                                   const char *lhs,
                                   int rhs) {
...
static void add_int_int_binary(maelys_datalog_internal_edb_t *edb,
                               const char *predicate,
                               long long lhs,
                               long long rhs) {
    maelys_datalog_internal_term_t terms[2];
...
static int query_solved_int_unary(const maelys_datalog_internal_solve_result_t *result,
                                  const char *predicate,
                                  long long value,
                                  bool *present) {
    maelys_datalog_internal_term_t term = int_term(value);
    return maelys_datalog_query_solved_ground_fact(result, predicate, &term, 1, present);
}

static int query_solved_int_binary(const maelys_datalog_internal_solve_result_t *result,
                                   const char *predicate,
                                   long long lhs,
                                   long long rhs,
                                   bool *present) {
    maelys_datalog_internal_term_t terms[2] = {int_term(lhs), int_term(rhs)};
    return maelys_datalog_query_solved_ground_fact(result, predicate, terms, 2, present);
}

static int query_solved_symbol_unary(maelys_datalog_internal_ruleset_t *r,
                                     const maelys_datalog_internal_solve_result_t *result,
                                     const char *predicate,
                                     const char *value,
                                     bool *present) {
    maelys_datalog_internal_term_t term = sym_term(r, value);
    return maelys_datalog_query_solved_ground_fact(result, predicate, &term, 1, present);
}

static int fact_array_contains_semantic(const maelys_datalog_internal_fact_t *facts,
                                        size_t count,
                                        const maelys_datalog_internal_fact_t *needle) {
...
static int solve_results_byte_identical(const maelys_datalog_internal_solve_result_t *lhs,
...
static int make_fact2(maelys_datalog_internal_ruleset_t *r,
                      const char *predicate,
                      const char *a,
                      const char *b,
                      maelys_datalog_internal_fact_t *out) {
...
static int solve_ancestor_chain(maelys_datalog_internal_ruleset_t *r,
                                maelys_datalog_internal_edb_t *edb,
                                maelys_datalog_internal_fact_t facts[8],
                                maelys_datalog_internal_solve_result_t **out_result) {
    const char *src =
...
static int test_solver_simple_and_query(void) {
    ...
    maelys_datalog_internal_term_t arg = sym_term(&r, "proj-1");
    bool present = false;
...
static int test_solver_preinterned_unary_symbol_id_equivalent_to_string_path(void) {
    ...
    maelys_datalog_internal_ruleset_t string_ruleset;
    maelys_datalog_internal_ruleset_t id_ruleset;
    TEST_ASSERT_EQUAL(MAELYS_OK, make_ruleset(&string_ruleset, src), "%d");
    TEST_ASSERT_EQUAL(MAELYS_OK, make_ruleset(&id_ruleset, src), "%d");

    maelys_datalog_internal_fact_t string_facts[4];
    maelys_datalog_internal_fact_t id_facts[4];
    maelys_datalog_internal_edb_t string_edb;
    maelys_datalog_internal_edb_t id_edb;
    TEST_ASSERT_EQUAL(MAELYS_OK,
    ...
    maelys_datalog_internal_solve_result_t *string_result = NULL;
    maelys_datalog_internal_solve_result_t *id_result = NULL;
...
static int test_solver_preinterned_binary_symbol_ids_equivalent_to_low_level_path(void) {
    ...
    maelys_datalog_internal_ruleset_t low_ruleset;
    maelys_datalog_internal_ruleset_t id_ruleset;
    TEST_ASSERT_EQUAL(MAELYS_OK, make_ruleset(&low_ruleset, src), "%d");
    TEST_ASSERT_EQUAL(MAELYS_OK, make_ruleset(&id_ruleset, src), "%d");

    maelys_datalog_internal_fact_t low_facts[4];
    maelys_datalog_internal_fact_t id_facts[4];
    maelys_datalog_internal_edb_t low_edb;
    maelys_datalog_internal_edb_t id_edb;
    TEST_ASSERT_EQUAL(MAELYS_OK,
    ...
    TEST_ASSERT_EQUAL(MAELYS_OK,
                      ...
                      "%d");
    maelys_datalog_internal_term_t low_terms[2] = {
    ...
    maelys_datalog_internal_solve_result_t *low_result = NULL;
    maelys_datalog_internal_solve_result_t *id_result = NULL;
...
static int test_solver_batch_unary_symbol_ids_equivalent_to_unit_insertion(void) {
    TEST_BEGIN();
    const char *src = "allow(U) :- user(U).";
    maelys_datalog_internal_ruleset_t batch_ruleset;
    maelys_datalog_internal_ruleset_t unit_ruleset;
    TEST_ASSERT_EQUAL(MAELYS_OK, make_ruleset(&batch_ruleset, src), "%d");
    TEST_ASSERT_EQUAL(MAELYS_OK, make_ruleset(&unit_ruleset, src), "%d");

    maelys_datalog_internal_fact_t batch_facts[8];
    maelys_datalog_internal_fact_t unit_facts[8];
    maelys_datalog_internal_edb_t batch_edb;
    maelys_datalog_internal_edb_t unit_edb;
    TEST_ASSERT_EQUAL(MAELYS_OK,
    ...
    maelys_datalog_internal_solve_result_t *batch_result = NULL;
    maelys_datalog_internal_solve_result_t *unit_result = NULL;
...
static int test_solver_positive_recursion_transitive_closure(void) {
    ...
    maelys_datalog_internal_solve_result_t *result = NULL;
    TEST_ASSERT_EQUAL(MAELYS_OK, maelys_datalog_solve_once(&r, &edb, &result), "%d");
    maelys_datalog_internal_term_t args[2] = {sym_term(&r, "alice"), sym_term(&r, "dave")};
    bool present = false;
    TEST_ASSERT_EQUAL(MAELYS_OK,
...
static int test_solver_duplicate_and_overflow_bounds(void) {
    ...
    maelys_datalog_internal_solve_result_t *result = NULL;
    TEST_ASSERT_EQUAL(MAELYS_OK, maelys_datalog_solve_once(&r, &edb, &result), "%d");
    maelys_datalog_internal_term_t args[2] = {sym_term(&r, "alice"), sym_term(&r, "bob")};
    bool present = false;
    TEST_ASSERT_EQUAL(MAELYS_OK,
                      maelys_datalog_query_solved_ground_fact(result, "ancestor", args, 2, &present),
                      "%d");
    ...
    maelys_datalog_internal_ruleset_t r2;
    TEST_ASSERT_EQUAL(MAELYS_OK, make_ruleset(&r2, "ancestor(X, Y) :- parent(X, Y)."), "%d");
    maelys_datalog_internal_fact_t facts2[4];
    maelys_datalog_internal_edb_t edb2;
    ...
    maelys_datalog_internal_solve_result_t *result2 = NULL;
...
static int test_solver_query_and_conflict_helper(void) {
    ...
    maelys_datalog_internal_term_t var = {.kind = MAELYS_DATALOG_TERM_VAR};
...
static int test_datalog_edb_dense_ranges_match_sorted_snapshot(void) {
    ...
    const maelys_datalog_internal_fact_t *slice = (const maelys_datalog_internal_fact_t *)1;
    size_t slice_count = 99;
    TEST_ASSERT_EQUAL(MAELYS_OK,
...
static int test_datalog_edb_dense_ranges_match_full_scan_reference(void) {
    ...
    maelys_datalog_internal_solve_result_t *optimized = NULL;
    maelys_datalog_internal_solve_result_t *full_scan = NULL;
    maelys_datalog_internal_solve_result_t *legacy = NULL;
    maelys_datalog_internal_solve_result_t *legacy_full_scan = NULL;
...
static int test_maelys_datalog_cartesian_product_within_bounds(void) {
    ...
    for (size_t u = 0; u < sizeof(users) / sizeof(users[0]); u++) {
        for (size_t q = 0; q < sizeof(resources) / sizeof(resources[0]); q++) {
            maelys_datalog_internal_term_t args[2] = {sym_term(&r, users[u]), sym_term(&r, resources[q])};
            bool present = false;
            TEST_ASSERT_EQUAL(MAELYS_OK,
...
static int test_maelys_datalog_cartesian_product_overflow_fails_closed(void) {
    ...
    maelys_datalog_internal_fact_t facts[2u * (MAELYS_DATALOG_MAX_FACTS_PER_PRED + 1u)];
    maelys_datalog_internal_edb_t edb;
    TEST_ASSERT_EQUAL(MAELYS_OK,
                      maelys_datalog_edb_init(&edb,
                                              facts,
...
static int test_datalog_allowed_backend_tuple_runtime_edb_rejected(void) {
    ...
    maelys_datalog_internal_term_t terms[3] = {
...
static int test_datalog_edb_mutation_after_solve_rejected(void) {
    ...
    maelys_datalog_internal_solve_result_t *result = NULL;
    TEST_ASSERT_EQUAL(MAELYS_OK, maelys_datalog_solve_once(&r, &edb, &result), "%d");
    maelys_datalog_internal_term_t terms[2] = {int_term(2), sym_term(&r, "b")};
...
static int test_datalog_ruleset_shared_across_two_solves(void) {
    ...
    maelys_datalog_internal_ruleset_t r;
    TEST_ASSERT_EQUAL(MAELYS_OK, make_ruleset(&r, "p(X) :- q(X, _)."), "%d");
    maelys_datalog_internal_fact_t facts1[4], facts2[4];
    maelys_datalog_internal_edb_t edb1, edb2;
    ...
    TEST_ASSERT_EQUAL(MAELYS_OK, maelys_datalog_edb_finalize(&edb2), "%d");
    maelys_datalog_internal_solve_result_t *r1 = NULL;
    maelys_datalog_internal_solve_result_t *r2 = NULL;
...
static non_int_ordinal_case_result_t run_non_int_ordinal_case(void) {
    ...
    add_int_symbol_binary(&r, &edb, "q", 1, "a");
    maelys_datalog_internal_term_t terms[2] = {sym_term(&r, "alice"), sym_term(&r, "b")};
...
typedef struct {
    maelys_datalog_internal_term_kind_t lhs_kind;
    maelys_datalog_cmp_op_t op;
    maelys_datalog_internal_term_kind_t rhs_kind;
    int expect_accept;
} comparison_matrix_case_t;

static const char *comparison_kind_literal(maelys_datalog_internal_term_kind_t kind, int rhs) {
    switch (kind) {
...
static int comparison_matrix_accepts(maelys_datalog_internal_term_kind_t lhs_kind,
                                     maelys_datalog_cmp_op_t op,
                                     maelys_datalog_internal_term_kind_t rhs_kind) {
...
static int test_comparison_parser_ground_matrix_54(void) {
    TEST_BEGIN();
    const maelys_datalog_internal_term_kind_t kinds[] = {
...
static int test_symbol_table_immutable_at_query_time(void) {
    ...
    maelys_datalog_internal_term_t alice = sym_term(&r, "alice");
    maelys_datalog_internal_term_t bob = sym_term(&r, "bob");
    TEST_ASSERT_EQUAL(MAELYS_OK, maelys_datalog_edb_finalize(&edb), "%d");
...
static int test_datalog_solve_once_symbol_eq_uses_symbol_id(void) {
    ...
    maelys_datalog_internal_edb_t edb;
    TEST_ASSERT_EQUAL(MAELYS_OK, maelys_datalog_edb_init(&edb, facts, 4, &r.symbols, &r.registry), "%d");
    maelys_datalog_internal_term_t alice[2] = {sym_term(&r, "alice"), sym_term(&r, "a")};
    maelys_datalog_internal_term_t bob[2] = {sym_term(&r, "bob"), sym_term(&r, "b")};
...
static int test_datalog_solve_once_unknown_term_kind_classified(void) {
    ...
    maelys_datalog_internal_ruleset_t r;
    TEST_ASSERT_EQUAL(MAELYS_OK, make_ruleset(&r, "p(X) :- q(X, _), X = 1."), "%d");
    r.rules[0].body[1].rhs.kind = (maelys_datalog_internal_term_kind_t)99;
    maelys_datalog_internal_fact_t facts[2];
    maelys_datalog_internal_edb_t edb;
    TEST_ASSERT_EQUAL(MAELYS_OK, maelys_datalog_edb_init(&edb, facts, 2, &r.symbols, &r.registry), "%d");
    add_int_symbol_binary(&r, &edb, "q", 1, "a");
...
static int test_solve_once_diag_invalid_state(void) {
    ...
    TEST_ASSERT_EQUAL(MAELYS_OK, maelys_datalog_edb_finalize(&edb), "%d");
    maelys_datalog_internal_solve_diagnostic_t diag;
    maelys_datalog_internal_solve_result_t *result = (maelys_datalog_internal_solve_result_t *)&diag;
    TEST_ASSERT_EQUAL(MAELYS_ERR_INVALID_STATE,
                      maelys_datalog_solve_once_ex(&r, &edb, &result, &diag),
                      "%d");
...
static int test_datalog_factstore_rejects_var_in_ground_fact(void) {
    ...
    maelys_datalog_internal_edb_t edb;
    TEST_ASSERT_EQUAL(MAELYS_OK, maelys_datalog_edb_init(&edb, facts, 2, &r.symbols, &r.registry), "%d");
    maelys_datalog_internal_term_t terms[2] = {{.kind = MAELYS_DATALOG_TERM_VAR}, sym_term(&r, "a")};
...
static int test_datalog_wildcard_not_query_wildcard(void) {
    ...
    maelys_datalog_internal_solve_result_t *result = NULL;
    TEST_ASSERT_EQUAL(MAELYS_OK, maelys_datalog_solve_once(&r, &edb, &result), "%d");
    maelys_datalog_internal_term_t wildcard_like_query_term = {
...
static int test_datalog_static_join_order_differential_source_vs_ordered(void) {
    ...
    TEST_ASSERT_EQUAL(MAELYS_OK, maelys_datalog_solve_once(&r, &edb, &ordered), "%d");
    TEST_ASSERT_EQUAL(MAELYS_OK, maelys_datalog_test_solve_once_legacy_order(&r, &edb, &legacy, NULL), "%d");
    const maelys_datalog_internal_fact_t *ordered_facts = NULL;
    const maelys_datalog_internal_fact_t *legacy_facts = NULL;
...
static int test_datalog_negation_stratified_positive_subset_equivalence(void) {
    TEST_BEGIN();
    maelys_datalog_internal_ruleset_t positive;
    maelys_datalog_internal_ruleset_t stratified;
    ...
    maelys_datalog_internal_fact_t positive_facts_pool[8];
    maelys_datalog_internal_fact_t stratified_facts_pool[8];
    maelys_datalog_internal_edb_t positive_edb;
    maelys_datalog_internal_edb_t stratified_edb;
    TEST_ASSERT_EQUAL(MAELYS_OK,
    ...
    maelys_datalog_internal_solve_result_t *positive_result = NULL;
    maelys_datalog_internal_solve_result_t *stratified_result = NULL;
    ...
    const maelys_datalog_internal_fact_t *positive_facts = NULL;
    const maelys_datalog_internal_fact_t *stratified_facts = NULL;
...
static int test_datalog_negation_decision_policy_compatible(void) {
    ...
    TEST_ASSERT_EQUAL(MAELYS_OK, maelys_datalog_solve_once(&r, &edb, &result), "%d");
    bool present = false;
    maelys_datalog_internal_term_t request = sym_term(&r, "request");
    TEST_ASSERT_EQUAL(MAELYS_OK,
                      maelys_datalog_query_solved_ground_fact(result,
                                                              "deny",
                                                              &request,
...
static int test_datalog_proof_idb_proof_index_survives_stratum_sort(void) {
    ...
    maelys_datalog_internal_solve_result_t *result = NULL;
    TEST_ASSERT_EQUAL(MAELYS_OK, maelys_datalog_solve_once(&r, &edb, &result), "%d");
    const maelys_datalog_internal_fact_t *idb = NULL;
...
static int make_fact1(maelys_datalog_internal_ruleset_t *r,
                      const char *predicate,
                      const char *a,
                      maelys_datalog_internal_fact_t *out) {
    memset(out, 0, sizeof(*out));
...
static int make_fact1_int(maelys_datalog_internal_ruleset_t *r,
                          const char *predicate,
                          long long value,
                          maelys_datalog_internal_fact_t *out) {
    memset(out, 0, sizeof(*out));
...
static const maelys_datalog_explanation_step_t *exp_find_step(
    ...
    const maelys_datalog_internal_fact_t *f) {
...
static int test_p4c64_two_edb_premises_lexical(void) {
    ...
    maelys_datalog_internal_fact_t cand;
    maelys_datalog_internal_fact_t usr;
...
static int test_p4c64_two_idb_premises_distinct_parents(void) {
    ...
    maelys_datalog_internal_fact_t target;
    maelys_datalog_internal_fact_t left_fact;
    maelys_datalog_internal_fact_t right_fact;
...
static int test_p4c64_negation_satisfied_ground_atom(void) {
    ...
    maelys_datalog_internal_fact_t target;
    maelys_datalog_internal_fact_t blocked_ground;
...
static int build_multi_witness(maelys_datalog_internal_ruleset_t *r,
                               maelys_datalog_internal_edb_t *edb,
                               maelys_datalog_internal_fact_t facts[8],
                               maelys_datalog_internal_solve_result_t **out_result) {
    if (make_ruleset(r,
...
static int test_p4c64_multiple_candidates_same_witness_two_solves(void) {
    TEST_BEGIN();
    maelys_datalog_internal_ruleset_t r1;
    maelys_datalog_internal_ruleset_t r2;
    maelys_datalog_internal_fact_t f1[8];
    maelys_datalog_internal_fact_t f2[8];
    maelys_datalog_internal_edb_t e1;
    maelys_datalog_internal_edb_t e2;
    maelys_datalog_internal_solve_result_t *r1res = NULL;
    maelys_datalog_internal_solve_result_t *r2res = NULL;
...
static int test_p4c64_proof_capacity_truncated_atomic(void) {
    ...
    for (size_t i = 0; i < fact_count; i++) {
        if (indices[i] != MAELYS_DATALOG_PROOF_NO_PARENT) continue;
        found_truncated = 1;
        maelys_datalog_internal_fact_t target = idb[i];
...
static int test_p4c64_error_paths_preserve_output(void) {
    ...
    maelys_datalog_internal_solve_result_t *result = NULL;
    TEST_ASSERT_TRUE(build_multi_witness(&r, &edb, facts, &result));
    maelys_datalog_internal_fact_t valid;
...
static int test_p4c64_ordered_and_reference_traversals_agree(void) {
    ...
    maelys_datalog_internal_solve_result_t *ordered = NULL;   /* production static join order */
    maelys_datalog_internal_solve_result_t *reference = NULL; /* reference recursive traversal */
