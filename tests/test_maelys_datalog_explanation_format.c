...
static int test_fmt_filter_true_name_semantic_value_and_pattern(void) {
    TEST_BEGIN();
    TEST_ASSERT_TRUE(ensure_fx());
    g_bad_rs = g_fx;
    g_bad_rs.filter_program_count = 1u;
    g_bad_rs.filter_pattern_pool_used = 4u;
    memcpy(g_bad_rs.filter_pattern_pool, "docs", 4u);
    memset(&g_bad_rs.filter_programs[0], 0,
           sizeof(g_bad_rs.filter_programs[0]));
    g_bad_rs.filter_programs[0].kind =
        (uint8_t)MAELYS_DATALOG_FILTER_STARTS_WITH;
    g_bad_rs.filter_programs[0].pattern_length = 4u;

    memset(&g_exp, 0, sizeof(g_exp));
    g_exp.found = 1u;
    g_exp.step_count = 1u;
    g_exp.premise_count = 1u;
    set_step(&g_exp,
             0u,
             1u,
             f2(g_pid_allow, t_sym(g_sym_alice), t_sym(g_sym_doc)),
             0u,
             1u);
    maelys_datalog_explanation_premise_t *premise = &g_exp.premises[0];
    ...
    premise->as.filter.value = t_sym(g_sym_doc);
    premise->as.filter.filter_kind =
        (uint8_t)MAELYS_DATALOG_FILTER_STARTS_WITH;
    premise->as.filter.program_index = 0u;

    size_t required = 0u;
    TEST_ASSERT_EQUAL(MAELYS_OK,
                      maelys_datalog_format_explanation_text(
                          &g_bad_rs, &g_exp, NULL, 0u, &required), "%d");
    TEST_ASSERT_TRUE(required + 1u <= sizeof(g_text_a));
    TEST_ASSERT_EQUAL(MAELYS_OK,
                      maelys_datalog_format_explanation_text(
                          &g_bad_rs,
                          &g_exp,
                          ...
                          sizeof(g_text_a),
                          &required), "%d");
    TEST_ASSERT_NOT_NULL(strstr(g_text_a, "kind=filter-true"));
    TEST_ASSERT_NOT_NULL(strstr(g_text_a, "filter=\"starts_with\""));
    TEST_ASSERT_NOT_NULL(strstr(
        g_text_a, "semantic=\"string.starts-with.utf8-bytes-v1\""));
    TEST_ASSERT_NOT_NULL(strstr(g_text_a, "value=\"doc.pdf\""));
    TEST_ASSERT_NOT_NULL(strstr(g_text_a, "pattern=\"docs\""));
    TEST_END();
}
...
int main(int argc, char **argv) {
    test_case_t cases[] = {
        ...
        {"maelys_datalog_explanation_format/filter_true_name_semantic_value_and_pattern",
         TEST_MODE_NON_BLOCKING, test_fmt_filter_true_name_semantic_value_and_pattern},
    };
