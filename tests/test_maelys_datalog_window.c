...
static void query(fixture *f, const char *p, size_t n, int64_t a, int64_t b, int expected) {
    maelys_datalog_value_t v[] = {integer(a),integer(b)}; int present = -1;
...
static void oracle(fixture *f, const maelys_datalog_fact_t *facts, size_t count) {
...
static void push(fixture *f, const char *p, int64_t a, int64_t b, size_t n, uint32_t id) {
    maelys_datalog_value_t values[]={integer(a),integer(b)};
    uint32_t got=UINT32_MAX;
    OK(maelys_datalog_window_push(f->w,p,values,n,&got,NULL)); assert(got==id);
}
static void rejected(fixture *f, const char *p, const maelys_datalog_value_t *v,
    size_t n, maelys_datalog_status_t expected) {
    const maelys_datalog_fact_t *before, *after; size_t count, after_count;
...
static void aggregates_and_rejections(void) {
    ...
    maelys_datalog_value_t v[]={integer(0),integer(INT32_MAX)};
...
static void directed_limits(void) {
    ...
    maelys_datalog_value_t v[]={integer(0),integer(8)};
    ...
    maelys_datalog_value_t s[]={integer(0),symbol("x")};
...
static void explanations(void) {
    ...
    maelys_datalog_value_t v[]={integer(0),integer(7)};
...
static void generated_sequences(void) {
    ...
    maelys_datalog_fact_t fifo[7]={0}; size_t used=0; uint32_t seed=0x19750322u;
    for (uint32_t id=0;id<600;++id) {
        ...
        maelys_datalog_value_t v[]={integer((seed>>8)%2),integer((seed>>16)%7)};
        ...
        fifo[used]=(maelys_datalog_fact_t){.predicate=p,.arity=arity};
        fifo[used].terms[0]=integer(id); fifo[used].terms[1]=v[0]; if(arity==3)fifo[used].terms[2]=v[1]; ++used;
        oracle(&f,fifo,used);
        if (id%37==0) {
            maelys_datalog_value_t bad[]={integer(0),integer(INT32_MAX)};
    ...
    maelys_datalog_fact_t bad={.predicate="event",.arity=3,.terms={integer(999),integer(0),integer(123)}};
...
static void full_arity_typed_events(void) {
    ...
    init(&f,3,64,0);
    maelys_datalog_value_t values[]={integer(1),integer(2),symbol("1")};
    maelys_datalog_fact_t fifo[3]={0};
    for(uint32_t i=0;i<3;++i) {
        ...
        fifo[i]=(maelys_datalog_fact_t){.predicate="mixed",.arity=4,
            .terms={integer(i),values[0],values[1],values[2]}};
    }
    oracle(&f,fifo,3); query(&f,"distinct",2,0,3,1);
    const maelys_datalog_fact_t *events; size_t count;
...
static void text_occupancy(void) {
    ...
    maelys_datalog_value_t v[]={integer(0),symbol("xx")};
...
static void renewable_snapshot_vocabulary(void) {
    ...
    char strings[3][32]; maelys_datalog_fact_t fifo[3]={0}; size_t used=0;
    for (uint32_t id=0;id<2*max+3;++id) {
        char incoming[32]; snprintf(incoming,sizeof(incoming),"symbol_%u",id);
        maelys_datalog_value_t v[]={integer(0),symbol(incoming)};
        OK(maelys_datalog_window_push(f.w,"event",v,2,NULL,NULL));
        if (used==3) { memmove(strings,strings+1,2*sizeof(*strings)); --used; }
        strcpy(strings[used++],incoming);
        for(size_t i=0;i<used;++i) fifo[i]=(maelys_datalog_fact_t){.predicate="event",.arity=3,
...
int main(void) {
    maelys_datalog_domain_t d={"window",predicates,sizeof(predicates)/sizeof(*predicates),NULL,0};
