...
Pipeline counters exist only in test-generated translation units. The build
uses `tests/helpers/instrument_pipeline.awk` to count five function entries and
fails if the expected definitions change. No replacement implementation is
written; production libraries and installed headers never contain counters.
The Make pipeline test observes parse/validation/fingerprint/preparation and
materialization calls. CMake's static/shared pipeline tests independently check
the same 21 transcript goldens and two filter validations without instrumentation.
The benchmark's CPU time is descriptive, not a throughput guarantee.
