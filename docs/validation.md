...
| `test_maelys_datalog_prepared_explanations` | All engine units use allocator hooks; Why-true/Why-false prepared and one-shot paths run with allocation disabled. One-shot short storage preserves the required-length output; short text reports its length, retry rebuilds, and write failures release the lease. Per-kind reference bounds dominate exact sizes, including both truncated kinds; copied descriptors are refused. Repeated prepared writes call no preparation callback; result leases, alignment, storage reuse, unknown symbols, invalid callbacks and ABI 2 rejection are checked. |
