...
static maelys_result_t canonical_update_bytes(maelys_sha256_ctx_t *ctx,
                                              const unsigned char *bytes,
                                              size_t length) {
    if (!ctx || (!bytes && length != 0u)) return MAELYS_ERR_INVALID_ARGUMENT;
    if (length != 0u) maelys_sha256_update(ctx, bytes, length);
    return MAELYS_OK;
}
...
static maelys_result_t canonical_stream_literal(maelys_sha256_ctx_t *ctx,
    ...
    switch (literal->kind) {
        ...
        case MAELYS_DATALOG_LITERAL_FILTER: {
            if (literal->filter_program_index >= ruleset->filter_program_count) {
                return MAELYS_ERR_INVALID_STATE;
            }
            ...
            if (!definition) return MAELYS_ERR_INVALID_STATE;
            maelys_result_t rc = canonical_printf(
                ctx,
                "body.filter=%s/%s:",
                definition->name,
                definition->semantic_id);
            if (rc != MAELYS_OK) return rc;
            rc = canonical_stream_term(ctx, ruleset, &literal->filter_value);
            if (rc != MAELYS_OK) return rc;
            rc = canonical_printf(ctx, ":pattern:%u:",
                                  (unsigned)program->pattern_length);
            if (rc != MAELYS_OK) return rc;
            rc = canonical_update_bytes(
                ctx,
...
static maelys_result_t finalize_authority(maelys_datalog_ruleset_t *ruleset) {
    ...
    int extended = ruleset->frontend_name[0] || maelys_datalog_context_planner(ruleset->modules) != NULL;
