...
TEST_ENGINE_SRCS = $(ENGINE_SRCS)
include tests/helpers/pipeline-build.mk
...
$(BUILD_DIR)/tests/test_maelys_datalog_modules: TEST_EXTRA_SRCS = examples/modules/exact_match.c
$(BUILD_DIR)/tests/test_maelys_datalog_modules: TEST_CFLAGS += -pthread
$(BUILD_DIR)/tests/test_maelys_datalog_modules: examples/modules/exact_match.c
$(BUILD_DIR)/tests/test_maelys_datalog_compiler: TEST_EXTRA_SRCS = examples/modules/arrow_frontend.c examples/modules/naive_backend.c
$(BUILD_DIR)/tests/test_maelys_datalog_compiler: examples/modules/arrow_frontend.c examples/modules/naive_backend.c
$(BUILD_DIR)/tests/test_maelys_datalog_pipeline: TEST_EXTRA_SRCS = examples/modules/arrow_frontend.c
$(BUILD_DIR)/tests/test_maelys_datalog_pipeline: examples/modules/arrow_frontend.c
$(BUILD_DIR)/tests/test_maelys_datalog_pipeline: TEST_ENGINE_SRCS = $(PIPELINE_ENGINE_SRCS)
$(BUILD_DIR)/tests/test_maelys_datalog_pipeline: $(PIPELINE_GENERATED_SRCS) tests/helpers/pipeline_counts.h
...
$(BUILD_DIR)/tests/%: tests/%.c $(SRCS) $(EXAMPLES_SRCS) $(TEST_HELPER_SRCS) $(WASM_TEST_SRCS) $(ENGINE_HEADERS) | $(BUILD_DIR)/tests
	$(CC) $(TEST_CFLAGS) -I. -Iinclude $(TEST_ENGINE_SRCS) $(COMMON_SRCS) $(EXAMPLES_SRCS) $(TEST_HELPER_SRCS) $(WASM_TEST_SRCS) $(TEST_EXTRA_SRCS) $< -o $@
