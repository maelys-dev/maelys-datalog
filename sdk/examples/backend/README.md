...
Computes a bounded positive Datalog fixed point through the public backend ABI. It intentionally does not advertise negation, filters or explanations. Unsupported required capabilities are rejected by the host. It is an educational oracle comparison, not a performance backend.

The public entrypoint `example_backend_extension()` returns the common declaration.
...
Twenty deterministic graphs, including an empty graph, with 500 ground reachability probes and enumeration-count comparisons against the reference.
