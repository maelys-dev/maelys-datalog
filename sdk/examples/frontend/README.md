...
Lowers unary `head <- body` lines and comments through the public IR builder. The host validates roles, bindings and capacities; `seed <- allow` is rejected because its head is EDB. This is deliberately not a Gitolite implementation.

The public entrypoint `example_frontend_extension()` returns the common declaration.
...
Valid lowering, syntax rejection and malformed-IR rejection through the real host.
